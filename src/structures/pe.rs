...
/// Stores info about the PE file header
pub struct PEHeader {
    pub machine: String,
}

/// Stores info about the PE file including size
pub struct PEFile {
    pub machine: String,
    pub size: usize,
}
...
/// Parse a PE file and calculate its total size
pub fn parse_pe_file(pe_data: &[u8]) -> Result<PEFile, StructureError> {
    const PE_MAGIC: usize = 0x00004550;
    ...
    // Size of PE header structure
    let pe_header_size = common::size(&pe_structure);
    let section_header_size = common::size(&section_header_structure);
                ...
                if let Ok(pe_header) = common::parse(pe_header_data, &pe_structure, "little") {
                    ...
                    if pe_header["magic"] == PE_MAGIC {
                        ...
                        if known_machine_types.contains_key(&pe_header["machine"]) {
                            ...
                            let optional_header_size = pe_header["optional_header_size"];

                            // Calculate the offset to the section headers
                            // Section headers start after: PE header + optional header
                            let sections_start = pe_header_end + optional_header_size;

                            // Find the highest offset + size from all sections
                            let mut max_offset: usize = sections_start;

                            for i in 0..number_of_sections {
                                let section_offset = sections_start + (i * section_header_size);
                                let section_end = section_offset + section_header_size;

                                if let Some(section_data) = pe_data.get(section_offset..section_end)
                                {
                                    if let Ok(section) = common::parse(
                                        section_data,
                                        &section_header_structure,
                                        "little",
                                    ) {
                                        let raw_data_ptr = section["pointer_to_raw_data"];
                                        let raw_data_size = section["size_of_raw_data"];

                                        // Only consider sections with raw data
                                        if raw_data_ptr > 0 && raw_data_size > 0 {
                                            let section_end_offset = raw_data_ptr + raw_data_size;
